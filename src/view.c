...
/*   Updated: 2023/11/06 21:18:37 by andde-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void view_set_background_color(t_view *view, int color)
{
	int i;
	int j;
	...
	while (++i < view->size.x)
	{
		...
		{
			if (i == 0 || j == 0)
				my_mlx_pixel_put(view, i, j, 0x000000);
			else
...
t_view view_init(void *mlx_ptr, t_v2i size, int color)
{
	...
	view.addr = mlx_get_data_addr(view.img,
								  ...
								  &view.endian);
	view.size = size;
	view_set_background_color(&view, color);
	return (view);
}
